...
-- Bookkeeping/audit only — who actually gets paid is always recomputed at
-- distribute-time from FableLeaderboard's on-chain score history (see
-- app/api/admin/campaigns), never read back out of these tables. No public
-- RLS policy is created for either table: only the Supabase service role
-- (via SUPABASE_SERVICE_ROLE_KEY, used server-side by the admin API routes)
-- can read/write them. If SUPABASE_SERVICE_ROLE_KEY isn't set, the admin
-- campaign routes will fail closed rather than fall back to the anon key.
...
CREATE TABLE IF NOT EXISTS public.campaigns (
    ...
    pool_avax       NUMERIC     NOT NULL CHECK (pool_avax > 0),
...
CREATE TABLE IF NOT EXISTS public.campaign_payouts (
    ...
    amount_avax     NUMERIC     NOT NULL,
    tx_hash         TEXT        NOT NULL,
    paid_at         TIMESTAMPTZ NOT NULL DEFAULT now(),

    UNIQUE (campaign_id, wallet_address)      -- a wallet is paid at most once per campaign
);
